...
const TABS: Array<{ key: string; label: string; href: (id: string) => string; match: (p: string) => boolean }> = [
  ...
  { key: 'chapters', label: '章节', href: (id) => `/projects/${id}/chapters`, match: (p) => p.includes('/chapters') || p.match(/\/projects\/[^/]+$/) !== null },
  { key: 'consistency', label: '一致性', href: (id) => `/projects/${id}/consistency`, match: (p) => p.includes('/consistency') },
];

export default function ProjectNav({ projectId, projectTitle }: { projectId: string; projectTitle: string }) {
